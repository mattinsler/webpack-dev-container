...
const Header = {
  ...
  decode(buffer: Buffer): HeaderData {
    if (buffer.length < Header.length) { throw new Error('Header is invalid size'); }
...
export class ObjectEncodeStream extends Transform {
...
export class ObjectDecodeStream extends Transform {
  ...
  _transform(chunk: any, encoding: string, callback: Function): void {
    ...
    if (this.header && this.buffer.length >= Header.length + this.header.length) {
      const decoded = this.encoder.decode(this.buffer.slice(Header.length, Header.length + this.header.length), this.header.version);
